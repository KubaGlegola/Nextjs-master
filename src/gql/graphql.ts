		...
		edges: Array<{
			node: {
				...
				product: {
					...
					name: string;
					price: number;
					images: { edges: Array<{ node: { url: string; alt: string } }> };
				};
			};
		}>;
	};
...
export type ProductFragment = {
	...
	reviews: {
		totalCount: number;
		edges: Array<{
			node: { id: string; author: string; title: string; description: string; rating: number; email: string };
		}>;
	};
};
...
export type CategoryGetAllQuery = {
	categories: { edges: Array<{ node: { id: string; name: string; slug: string } }> };
};

export type CollectionsGetAllQueryVariables = Exact<{ [key: string]: never }>;

export type CollectionsGetAllQuery = {
	collections: { edges: Array<{ node: { id: string; name: string; description: string; slug: string } }> };
};
...
export type ProductsGetListBySearchQuery = {
	products: {
		totalCount: number;
		edges: Array<{
			node: {
